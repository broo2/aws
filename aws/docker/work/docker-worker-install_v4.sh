...
yum -y update
yum install -y python-pip git docker-18.03.1ce-5.0.amzn1 cifs-utils
...
cnappuser_pass=`aws ssm get-parameter --region $region --name $friendly_name-cnappuser_pass --output text --query "Parameter.Value"`
...
# Set cnappuser credentials
cnappuser_pass=`aws ssm get-parameter --region $region --name  $environment-cnappuser_pass --output text --query "Parameter.Value"`
echo -e "username=cnappuser\npassword=${cnappuser_pass}" > /root/.cifs_cred_cnappuser
chmod 600 /root/.cifs_cred_cnappuser

# Add entry to fstab if it doesn't already exist
if ! grep -q cashnet_file01_priv /etc/fstab ; then
   echo "//fileshare/cashnet_file01_priv /fileshare/cashnet_file01_priv cifs credentials=/root/.cifs_cred_cnappuser 0 0" >> /etc/fstab
fi

# Make the target directory and mount it
mkdir -p /fileshare/cashnet_file01_priv
mount /fileshare/cashnet_file01_priv
